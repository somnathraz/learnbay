...
const ThankYouDemo = () => {
  ...
  const { titleCourse, brochureLink } = router.query;
  console.log(brochureLink, titleCourse);
  const [data, setData] = useState("");
useEffect(() => {
  if(router.isReady){
    const { titleCourse, brochureLink } = router.query;
    {setData({titleCourse:titleCourse, brochureLink:brochureLink});
 }
   }
}, [router.isReady]);

  // useEffect(() => {setData({titleCourse:titleCourse, brochureLink:brochureLink});
  // }, []);
  console.log(data.titleCourse);
  return (
    <div>
      <FirstHeader brochureLink={data.brochureLink} donwnload={true} />
      <ThankStats
